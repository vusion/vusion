...
module.exports = function (dir) {
    ...
    if (hasImg(screenShotPath)) {
        screenShot = fs.readdirSync(screenShotPath).sort((a, b) => parseInt(a) - parseInt(b));
    }

    let drawingShot = [];

    const drawingShotPath = path.join(dir, 'drawings');

    if (hasSvg(drawingShotPath)) {
        drawingShot = fs.readdirSync(drawingShotPath).sort((a, b) => parseInt(a) - parseInt(b));
    }
    ...
    return {
        ...
        jsonSchema: {
            ...
            screenShot: JSON.stringify(screenShot),
            drawingShot: JSON.stringify(drawingShot),
            blocks,
            attrs: desc.attrs,
            slots: desc.slots,
